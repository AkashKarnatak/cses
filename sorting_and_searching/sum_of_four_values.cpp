...
public:
  ...
  Sum(int32_t i, int32_t j, int32_t v) : i(i), j(j), v(v) {}
...
int main() {
  ...
  [&]() {
    for (int32_t i = 0; i < n - 3; ++i) {
      for (int32_t j = i + 1; j < n - 2; ++j) {
        for (int32_t k = i + 2, l = n - 1; k < l;) {
          int64_t sum =
              (int64_t)v[i].first + v[j].first + v[k].first + v[l].first;
          if (sum == x) {
            cout << v[i].second + 1 << " " << v[j].second + 1 << " "
                 << v[k].second + 1 << " " << v[l].second + 1 << endl;
            return;
          } else if (sum > x) {
            --l;
          } else {
